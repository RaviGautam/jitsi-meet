...
const ParticipantContextMenu = ({
    ...
    const _isModerator = Boolean(
        localParticipant?.role === PARTICIPANT_ROLE.MODERATOR
    );
    const _isVideoForceMuted = useSelector<IReduxState>((state) =>
        isForceMuted(participant, MEDIA_TYPE.VIDEO, state)
    );
    const _isAudioMuted = useSelector((state: IReduxState) =>
        isParticipantAudioMuted(participant, state)
    );
    const _isVideoMuted = useSelector((state: IReduxState) =>
    ...
    const _volume =
        (participant?.local ?? true
            ? undefined
            : participant?.id
            ? participantsVolume[participant?.id]
            : undefined) ?? 1;
    const isBreakoutRoom = useSelector(isInBreakoutRoom);
    const isModerationSupported = useSelector((state: IReduxState) =>
        isAvModerationSupported()(state)
    );
    const stageFilmstrip = useSelector(isStageFilmstripAvailable);
    const shouldDisplayVerification = useSelector((state: IReduxState) =>
        displayVerification(state, participant?.id)
    );
    ...
    const _getCurrentParticipantId = useCallback(() => {
        ...
        return (
            (drawer ? drawerParticipant?.participantID : participant?.id) ?? ""
        );
    ...
    if (_isModerator) {
        if (isModerationSupported) {
            ...
            if (
                ...
                !(
                    isClickedFromParticipantPane &&
                    quickActionButtonType === QUICK_ACTION_BUTTON.ALLOW_VIDEO
                )
            ) {
                buttons.push(
                    <AskToUnmuteButton
        ...
        if (!disableRemoteMute) {
            if (
                !(
                    isClickedFromParticipantPane &&
                    quickActionButtonType === QUICK_ACTION_BUTTON.MUTE
                )
            ) {
            ...
            buttons.push(
                <MuteEveryoneElseButton
                    {...getButtonProps(BUTTONS.MUTE_OTHERS)}
                />
            );
            if (
                !(
                    isClickedFromParticipantPane &&
                    quickActionButtonType === QUICK_ACTION_BUTTON.STOP_VIDEO
                )
            ) {
                buttons.push(
                    <MuteVideoButton {...getButtonProps(BUTTONS.MUTE_VIDEO)} />
                );
            }
            buttons.push(
                <MuteEveryoneElsesVideoButton
                    {...getButtonProps(BUTTONS.MUTE_OTHERS_VIDEO)}
                />
            );
        }
        ...
        if (!disableGrantModerator && !isBreakoutRoom) {
            buttons2.push(
                <GrantModeratorButton
                    {...getButtonProps(BUTTONS.GRANT_MODERATOR)}
                />
            );
        }
        ...
        if (!disableDemote && visitorsSupported && _isModerator) {
            buttons2.push(
                <DemoteToVisitorButton {...getButtonProps(BUTTONS.DEMOTE)} />
            );
        }
        ...
        if (shouldDisplayVerification) {
            buttons2.push(
                <VerifyParticipantButton {...getButtonProps(BUTTONS.VERIFY)} />
            );
        }
    }
    ...
    if (stageFilmstrip) {
        buttons2.push(
            <TogglePinToStageButton {...getButtonProps(BUTTONS.PIN_TO_STAGE)} />
        );
    }
    ...
    if (!disablePrivateChat && !visitorsMode) {
        buttons2.push(
            <PrivateMessageMenuButton
                {...getButtonProps(BUTTONS.PRIVATE_MESSAGE)}
            />
        );
    }
    ...
    if (thumbnailMenu && isMobileBrowser()) {
        buttons2.push(
            <ConnectionStatusButton {...getButtonProps(BUTTONS.CONN_STATUS)} />
        );
    }
    ...
    return (
        ...
        >
            ...
            {buttons.length > 0 && (
                <ContextMenuItemGroup>{buttons}</ContextMenuItemGroup>
            )}
