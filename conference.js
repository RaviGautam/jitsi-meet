...
import {
    ...
    submitFeedback,
} from "./react/features/feedback/actions";
...
function setGUMPendingStateOnFailedTracks(tracks) {
    ...
    const nonPendingTracks = [MEDIA_TYPE.AUDIO, MEDIA_TYPE.VIDEO].filter(
        (type) => !tracksTypes.includes(type)
    );
...
export default {
    ...
    createInitialLocalTracks(options = {}) {
        ...
        const initialDevices =
            config.startSilent || config.disableInitialGUM
                ? []
                : [MEDIA_TYPE.AUDIO];
        ...
        if (options.startScreenSharing && config._desktopSharingSourceDevice) {
            ...
            APP.store.dispatch(
                gumPending(initialDevices, IGUMPendingState.PENDING_UNMUTE)
            );
            tryCreateLocalTracks = createLocalTracksF({
                ...
                if (
                    error.name === JitsiTrackErrors.TIMEOUT &&
                    !browser.isElectron()
                ) {
                ...
                results.forEach((result, idx) => {
                    if (result.status === "fulfilled") {
                        ...
                        logger.error(
                            `${
                                isAudio ? "Audio" : "Video"
                            } track creation failed with error ${errorMsg}`
                        );
                        if (isAudio) {
        ...
        return Promise.all([
            tryCreateLocalTracks
                ...
                .then((tr) => {
                    ...
                    const filteredTracks = handleInitialTracks(
                        initialOptions,
                        tr
                    );
    ...
    muteAudio(mute, showUI = true) {
        ...
        if (!localAudio && !mute) {
            ...
            APP.store.dispatch(
                gumPending([MEDIA_TYPE.AUDIO], IGUMPendingState.PENDING_UNMUTE)
            );
            createLocalTracksF({ devices: ["audio"] })
                ...
                .finally(() => {
                    APP.store.dispatch(
                        gumPending([MEDIA_TYPE.AUDIO], IGUMPendingState.NONE)
                    );
                });
        } else {
            muteLocalAudio(mute);
    ...
    muteVideo(mute, showUI = true) {
        ...
        if (!localVideo && !mute && !this.isCreatingLocalTrack) {
            ...
            APP.store.dispatch(
                gumPending([MEDIA_TYPE.VIDEO], IGUMPendingState.PENDING_UNMUTE)
            );
            ...
            createLocalTracksF({ devices: ["video"] })
                ...
                .finally(() => {
                    ...
                    APP.store.dispatch(
                        gumPending([MEDIA_TYPE.VIDEO], IGUMPendingState.NONE)
                    );
                });
        } else {
    ...
    _createRoom(localTracks) {
        ...
        if (!browser.isWebKitBased()) {
            ...
            APP.store.dispatch(
                gumPending(mutedTrackTypes, IGUMPendingState.NONE)
            );
        }
    ...
    _setLocalAudioVideoStreams(tracks = []) {
        ...
        return Promise.allSettled(promises).then(() => {
            if (pendingGUMDevicesToRemove.length > 0) {
                dispatch(
                    gumPending(pendingGUMDevicesToRemove, IGUMPendingState.NONE)
                );
            }
    ...
    _setupListeners() {
        ...
        if (!config.tokenAuthUrl) {
            room.on(
                ...
                (authEnabled, authLogin) =>
                    APP.store.dispatch(
                        authStatusChanged(authEnabled, authLogin)
                    )
            );
        }
        ...
        APP.UI.addListener(UIEvents.VIDEO_DEVICE_CHANGED, (cameraDeviceId) => {
            ...
            const localVideoTrack = getLocalJitsiVideoTrack(
                APP.store.getState()
            );
    ...
    async _onDeviceListChanged(devices) {
        ...
        if (
            !areDevicesDifferent(
                flattenAvailableDevices(oldDevices),
                filteredDevices
            )
        ) {
            return Promise.resolve();
        ...
        if (requestedInput.audio || requestedInput.video) {
            ...
            for (const track of tracks) {
                if (track.isAudioTrack()) {
                    ...
                    promises.push(this.useVideoStream(track));
                }
            }
        }
    ...
    hangup(requestFeedback = false, hangupReason) {
        ...
        if (requestFeedback) {
            const feedbackDialogClosed = (feedbackResult = {}) => {
                if (!feedbackResult.wasDialogShown && hangupReason) {
                    return APP.store
                        .dispatch(openLeaveReasonDialog(hangupReason))
                        .then(() => feedbackResult);
                }
            ...
            feedbackResultPromise = APP.store
                .dispatch(maybeOpenFeedbackDialog(room, hangupReason))
        ...
        Promise.allSettled([feedbackResultPromise, leavePromise]).then(
            ([feedback, _]) => {
                ...
                APP.store.dispatch(
                    maybeRedirectToWelcomePage(feedback.value ?? {})
                );
            }
        );
    },
